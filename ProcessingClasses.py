...
# fabiosacca, 2022-Dec-13, Modified DataProcessor to include add_track and select_cd methods
#------------------------------------------#
...
class DataProcessor:
    ...
    def add_CD(CDInfo, table):
        ...
        table.append(row)
        # @fabiosacca: need to include logic to prevent users from adding a cdID that already exists
    ...
    def add_track(track_info: tuple, cd: DC.CD) -> None:
        ...
        # TODone add code as required
        pos, ttl, lgth = track_info
        try:
            pos = int(pos)
        except:
            raise Exception('ID must be an Integer!')
        track = DC.Track(pos, ttl, lgth)
        cd.add_track(track)
